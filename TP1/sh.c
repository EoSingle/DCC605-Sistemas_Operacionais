...
/* MARK NAME Lucas Albano Olive Cruz */
...
runcmd(struct cmd *cmd)
{
  ...
  case '|':
    ...
    if (fork1() == 0) {    // Processo filho para o lado esquerdo do pipe
        close(1);          // Fecha stdout
        dup2(p[1], 1);     // Substitui stdout pelo pipe de escrita
        close(p[0]);       // Fecha o lado de leitura do pipe
        close(p[1]);
        runcmd(pcmd->left);
    }

    if (fork1() == 0) {    // Processo filho para o lado direito do pipe
        close(0);          // Fecha stdin
        dup2(p[0], 0);     // Substitui stdin pelo pipe de leitura
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "Erro ao mudar de diretório para %s\n", buf+3);
      continue;
    }
    /* MARK END task1 */
